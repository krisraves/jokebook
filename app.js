...
async function loadData() {
  ...
  for (const path of paths) {
    try {
      ...
      $('stats').textContent =
        `${Number(d.summary?.grouped_jokes || JOKES.length).toLocaleString()} jokes loaded • ` +
        `${Number(d.summary?.candidate_occurrences || 0).toLocaleString()} note occurrences • ` +
        `${Number(d.summary?.detected_setlists || 0).toLocaleString()} setlists`;
      ...
      console.warn('Failed to load joke data from:', path, err);
    }
  }

  $('stats').textContent = 'Unable to load joke database.';
}
...
function populateFilters() {
  ...
  addOptions(
    'comedianFilter',
    uniq(
      JOKES.map(j => j.sounds_like).concat(
        JOKES.flatMap(j =>
          (j.comedian_style_matches || []).map(c => c.comedian)
        )
      )
    )
  );
...
function current() {
  return filtered[pos] || filtered[0] || JOKES[0];
}
...
function render() {
  ...
  $('jokeId').textContent =
    `${j.id} • ${j.rating}/100 • ${pos + 1}/${filtered.length}`;
  ...
  $('analysis').innerHTML = `
    ...
    <div>
      ${[...(j.subject_tags || []), ...(j.style_tags || []), j.length_tag]
        .filter(Boolean)
    ...
    <div class="box">
      <b>Average comedy lover</b>
      <p>${esc(j.average_comedy_lover_analysis)}</p>
    </div>

    <div class="box">
      <b>30-year joke writer</b>
      <p>${esc(j.veteran_joke_writer_analysis)}</p>
    </div>

    <div class="box">
      <b>Three comedian-style lenses</b>
      ${(j.comedian_style_matches || [])
        .map(c => `<p><b>${esc(c.comedian)}</b>: ${esc(c.reason)}</p>`)
        .join('') || '<p>No clear match.</p>'}
    </div>

    <div class="box">
      <b>Demographic reads</b>
      ${Object.entries(j.demographic_comments || {})
    ...
    <div class="box">
      <b>Used in sets</b>
      <p>${j.set_usage?.length
        ? j.set_usage.map(esc).join('<br>')
        : 'No setlist match detected.'}</p>
    </div>
  `;

  $('vars').innerHTML = (j.all_variations || [])
    .map((v, i) =>
      `<div class="variation">
        <b>${i === 0 ? 'Best version' : 'Variation ' + i}</b>
        <p>${esc(v)}</p>
      </div>`
    )
    .join('');
...
$('prev').onclick = () => {
  ...
  pos = (pos - 1 + filtered.length) % filtered.length;
  render();
};
...
function applyFilters() {
  ...
  filtered = JOKES.filter(j =>
    ...
    (!com ||
      j.sounds_like === com ||
      (j.comedian_style_matches || []).some(c => c.comedian === com)) &&
  ...
  filtered.sort((a, b) => {
    if (s === 'best_version') {
      return dir * String(a.best_version).localeCompare(String(b.best_version));
    }
  ...
  $('filterCount').textContent =
    `${filtered.length} cards shown out of ${JOKES.length}`;
...
function renderIndex() {
  $('jokeIndex').innerHTML =
    filtered.slice(0, 900)
      .map((j, i) =>
        `<div class="idx" data-i="${i}">
          <b>${esc(j.id)}</b> ${esc(j.best_version)}
          <div class="small">
            ${j.rating}/100 • ${j.times_written}x • ${j.variation_count} variations • ${esc(j.sounds_like)}
          </div>
        </div>`
      )
      .join('') || '<p class="small">No cards match those filters.</p>';

  document.querySelectorAll('.idx').forEach(el => {
    el.onclick = () => {
      pos = Number(el.dataset.i);
      showTab('analysis');
      render();
...
$('speakBtn').onclick = () => {
  const u = new SpeechSynthesisUtterance(current().best_version);
  speechSynthesis.cancel();
  speechSynthesis.speak(u);
};

$('noteBox').oninput = () =>
  localStorage.setItem('note_' + current().id, $('noteBox').value);

$('addSet').onclick = () => {
  const j = current();

  if (!setlist.find(x => x.id === j.id)) {
    setlist.push({ id: j.id, text: j.best_version });
  }
...
function loadSet() {
  setlist = store.get('activeSetlist', []);
  renderSet();
}
...
function renderSet() {
  $('setItems').innerHTML = setlist.length
    ? setlist.map((j, i) =>
        `<div class="setrow">
          <span>${i + 1}</span>
          <span>${esc(j.text)}</span>
          <button data-move="${i}">↑</button>
          <button data-remove="${i}">×</button>
        </div>`
      ).join('')
    : '<p class="small">Add jokes from the book to build a set.</p>';

  $('cards').innerHTML = setlist
    .map((j, i) => `<div class="card">${i + 1}. ${esc(j.text)}</div>`)
    .join('');

  document.querySelectorAll('[data-move]').forEach(b => {
    b.onclick = () => move(Number(b.dataset.move), -1);
  });

  document.querySelectorAll('[data-remove]').forEach(b => {
    b.onclick = () => removeSet(Number(b.dataset.remove));
  });
}

function move(i, d) {
  const n = i + d;
  if (n < 0 || n >= setlist.length) return;

  [setlist[i], setlist[n]] = [setlist[n], setlist[i]];
  saveSetLocal();
  renderSet();
}
...
$('saveSet').onclick = () => {
  ...
  const saved = store.get('savedSets', {});
  saved[name] = setlist;

  store.set('savedSets', saved);
  renderSavedSets();
  toast('Saved setlist');
};
...
function renderSavedSets() {
  ...
  $('savedSets').innerHTML = Object.keys(saved).length
    ? Object.entries(saved).map(([name, items]) =>
        `<div class="saved">
          <b>${esc(name)}</b>
          <div class="small">${items.length} jokes</div>
          <button data-loadset="${esc(name)}">Load</button>
        </div>`
      ).join('')
    : '<p class="small">No saved setlists yet.</p>';

  document.querySelectorAll('[data-loadset]').forEach(b => {
    b.onclick = () => {
      setlist = saved[b.dataset.loadset] || [];
      saveSetLocal();
      renderSet();
      toast('Loaded setlist');
    };
  });
}

$('saveNew').onclick = () => {
  const text = $('newJoke').value.trim();
  if (!text) return;

  const drafts = store.get('draftJokes', []);
...
function renderDrafts() {
  ...
  $('drafts').innerHTML = drafts
    .map(d =>
      `<div class="variation">
        ${esc(d.text)}
        <div class="small">${esc(d.tags)} • ${new Date(d.created).toLocaleString()}</div>
      </div>`
    )
    .join('');
}
...
$('exportState').onclick = () => {
  const data = {
    ...
    savedSets: store.get('savedSets', {}),
    draftJokes: store.get('draftJokes', []),
  ...
  const a = Object.assign(document.createElement('a'), {
    ...
    download: 'joke-grimoire-app-data.json'
  });
...
$('clearState').onclick = () => {
  if (confirm('Clear saved setlists, draft jokes, and notes from this browser?')) {
    Object.keys(localStorage)
      .filter(k =>
        k.startsWith('note_') ||
        ['activeSetlist', 'savedSets', 'draftJokes'].includes(k)
      )
