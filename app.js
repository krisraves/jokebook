...
fetch('data/jokes.json').then(r => r.json()).then(d => {
  ...
  $('stats').textContent = `${d.summary.grouped_jokes} grouped jokes • ${d.summary.candidate_occurrences} note occurrences • ${d.summary.detected_setlists} detected setlists`;
  loadSet(); applyFilters(); renderDrafts(); renderSavedSets();
...
function render(){
  ...
  $('jokeId').textContent = `${j.id} • ${j.rating}/100`;
  ...
  $('analysis').innerHTML = `
    ...
    <div class="box"><b>Three comedian-style lenses</b>${(j.comedian_style_matches||[]).map(c => `<p><b>${esc(c.comedian)}</b>: ${esc(c.reason)}</p>`).join('')}</div>
  ...
  $('vars').innerHTML = (j.all_variations||[]).map(v => `<div class="variation">${esc(v)}</div>`).join('');
...
$('prev').onclick = () => { pos = (pos - 1 + filtered.length) % filtered.length; render(); };
$('next').onclick = () => { pos = (pos + 1) % filtered.length; render(); };
$('search').oninput = applyFilters; $('sort').onchange = applyFilters; $('maturity').onchange = applyFilters;
function applyFilters(){
  const q = $('search').value.toLowerCase(); const m = $('maturity').value;
  filtered = JOKES.filter(j => (!m || j.maturity_rating === m) && (!q || JSON.stringify(j).toLowerCase().includes(q)));
  const s = $('sort').value; filtered.sort((a,b) => (b[s] || 0) - (a[s] || 0)); pos = 0; render(); renderIndex();
}
function renderIndex(){
  $('jokeIndex').innerHTML = filtered.slice(0,700).map((j,i) => `<div class="idx" data-i="${i}"><b>${esc(j.id)}</b> ${esc(j.best_version)}<div class="small">${j.rating}/100 • ${j.times_written}x • ${esc(j.sounds_like)}</div></div>`).join('');
